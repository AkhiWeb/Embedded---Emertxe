        ...
        if(check_capacity(encInfo) == e_success)
        {
            ...
            if(copy_bmp_header(encInfo -> fptr_src_image, encInfo -> fptr_stego_image) == e_success)
            {
                ...
                if(encode_magic_string(MAGIC_STRING, encInfo) == e_success)
                {
                     ...
                     {
                        printf("Encoding size of extn is a success\n");
                        if(encode_secret_file_extn(encInfo -> extn_secret_file, encInfo) == e_success)
                        {
                          ...
                          if(encode_secret_file_size(encInfo -> size_secret_file, encInfo) == e_success)
                          {
                              ...
                              if(encode_secret_file_data(encInfo) == e_success)
                              {
                                 ...
                                 {
                                     printf("Copied remaining data succesfully\n");
                                 }
                                 else
                                 {
                                     printf("failed to copy remainng data\n");
                                     return e_failure;
                                 }
                              }
                              else
                          ...
                          {
                              printf("failed to encode size of the secret file\n");
                              return e_failure;
                          } 
                        }
                        else
                     ...
                     {
                         printf("Encoding size of extn is a failure\n");
                         return e_failure;
                     }
                }
                else
